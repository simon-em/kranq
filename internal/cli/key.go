...
func runKey(env Env, args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(env.Stderr, "usage: kranq key add|ls|rm")
	...
	sub, ok := subs[args[0]]
	if !ok {
		fmt.Fprintf(env.Stderr, "kranq key: unknown subcommand %q\n", args[0])
		return exitcode.Usage
	}
...
func keyAdd(env Env, args []string) int {
	if len(args) != 2 {
		fmt.Fprintln(env.Stderr, "usage: kranq key add <name> <path-to-public-key>")
		fmt.Fprintln(env.Stderr, "       kranq key add <name> -   to read it from stdin")
		return exitcode.Usage
	}
	...
	if err != nil {
		...
		if errors.Is(err, authkeys.ErrExists) {
			fmt.Fprintf(env.Stderr, "remove it first if you mean to replace it: kranq key rm %s\n", name)
		}
		return exitcode.Usage
	}
	...
	fmt.Fprintln(env.Stderr, "it can push to kranq and do nothing else: no shell, no forwarding")
	return exitcode.OK
}
...
func keyList(env Env, args []string) int {
	...
	if len(keys) == 0 {
		fmt.Fprintf(env.Stderr, "no kranq keys in %s\n", path)
		return exitcode.OK
	}
...
func keyRemove(env Env, args []string) int {
	if len(args) != 1 {
		fmt.Fprintln(env.Stderr, "usage: kranq key rm <name>")
		return exitcode.Usage
	}
	path, err := authkeys.Path()
	if err != nil {
