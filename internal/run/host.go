...
func HostCheckout(ctx context.Context, remote Remote, ref, dest string) error {
	...
	if remote.Token != "" {
		...
		args = append([]string{"-c", "credential.helper=",
			"-c", `credential.helper=!f() { echo username=x-token-auth; echo "password=$FORGE_GIT_TOKEN"; }; f`}, args...)
	}
	cmd := exec.CommandContext(ctx, "git", append(args, url, dest)...)
	cmd.Env = append(os.Environ(), "FORGE_GIT_TOKEN="+remote.Token, "GIT_TERMINAL_PROMPT=0")
